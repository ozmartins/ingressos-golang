// Package amqp é o adaptador de mensageria: topologia e consumo. Este serviço
// não publica fato nenhum (research.md D10), então não há publicador aqui.
package amqp
...
// Topologia é a declaração dos recursos AMQP (contracts/eventos.md §2).
...
func (t Topologia) Declarar(ch *amqp.Channel) error {
	...
	//
	// Tradução de vocabulário, herdada do Servico-Pagamento, que a mediu contra
	// o broker: a FR-022 fala em TENTATIVAS (padrão 3); o RabbitMQ trata
	// x-delivery-limit como número de REENTREGAS, entregando limite+1 vezes.
	// Subtrai-se 1 para que o teto configurado signifique o que a spec diz.
	// Reverificado em test/integration/emissao_test.go.
